...
from vending.database import db, Machine, Product, StockTimeline
...
def test_edit_machine_app(app, client):
    with app.app_context():
        machine = Machine(machine_name="Test Machine", location="Test Location")
        db.session.add(machine)
        db.session.commit()

        response = client.post("/edit-machine", data={"machine_id": machine.machine_id, "name": "New Name", "location": "New Location"})
...
def test_add_product_app(app, client):
    with app.app_context():
        machine = Machine(machine_name="Test Machine", location="Test Location")
        db.session.add(machine)
        db.session.commit()

        response = client.post("/add-product", data={"machine_id": machine.machine_id, "product_name": "Test Product", "amount": 10})
        assert response.json == status(0)
...
def test_edit_product_app(app, client):
    with app.app_context():
        machine = Machine(machine_name="Test Machine", location="Test Location")
        db.session.add(machine)
        db.session.commit()
        ...
        response = client.post("/edit-product", data={"product_id": product.product_id, "machine_id": machine.machine_id, "product_name": "New Product", "amount": 20})
        assert response.json == status(0)
...
def test_get_stock_timeline_app(app, client):
    with app.app_context():
        machine = Machine(machine_name="Test Machine", location="Test Location")
        db.session.add(machine)
        db.session.commit()
...
def test_get_product_timeline_app(app, client):
    with app.app_context():
        machine = Machine(machine_name="Test Machine", location="Test Location")
        db.session.add(machine)
        db.session.commit()
...
def test_get_machine_timeline_app(app, client):
    with app.app_context():
        machine = Machine(machine_name="Test Machine", location="Test Location")
        db.session.add(machine)
        db.session.commit()
